...
func TestRecognizeEventOnChunks(t *testing.T) {
	for _, tc := range []struct {
		...
		{"a tool call delta with arguments commits",
			`{"choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"function":{"name":"f","arguments":"{\"x\":1}"}}]}}]}`, true, false},
		{"a literal tool_calls null does not commit",
			`{"choices":[{"index":0,"delta":{"tool_calls":null}}]}`, false, false},
