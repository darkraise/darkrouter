...
- **`bytes`** renders `Input` seeded with `row.display` and emits `String(parseBytes(text))`; when `parseBytes` returns undefined it emits the raw text so the server can refuse it and say why, rather than the screen silently dropping the keystroke.
