...
| Settings header | Three actions now share the `PageHeader` row and the new one carries the longest label. Check it does not wrap or overflow at 1280×800, and again at the extra-large font-size step. Also judge whether the only destructive action of the three belongs first in the row, where it currently sits |
