...
Assessed 2026-08-27 on `feat/console-gap-closure`, against the Definition of
Done table in `docs/superpowers/plans/2026-08-27-phase14-console-gap-closure.md`.
...
- `go test -race -count=1 ./...` — **26 packages ok, 0 failures**, including
  ...
  `internal/store`.
- `cd web && npm test` — **203 passed in 30 files**.
...
| D3 | Token count shows native versus estimated | Command verified; UAT not performed | `go test ./internal/admin/ -run 'TestCountRequestBuild\|TestTheCountEndpoint' -v` | 4 tests ran, all passed. UAT half (`X-Darkrouter-Estimated` driving the marker text) not performed |
...
| D9 | Preset browser adds a provider and a credential without touching a file | Command verified; UAT not performed | `cd web && npm test -- add-provider` (covered within the full 203-test run; not isolated separately) | `add-provider-dialog.test.tsx` passes as part of the full suite. UAT full flow (browse → filter by surface → create → add credential → probe ok) not performed |
...
| D14 | Trace: waterfall, a Bodies panel that explains itself, surface metadata, Open-in-playground round-trips | Command verified; UAT not performed | `cd web && npm test -- trace-drawer` | `trace-drawer.test.tsx` passes as part of the full suite. UAT (playground run → trace → seed back into playground) not performed |
...
| D20 | Suites green | **Met** | `go vet ./... && go build ./...`; `go test -race -count=1 ./...`; `cd web && npm test && npm run typecheck && npm run build` | All clean. See Static verification above. This row carries no UAT half in the plan |
...
it.** No eslint configuration file exists anywhere under `web/`. Pre-existing,
not a gate this plan introduced or is required to close.
